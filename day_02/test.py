...
def test_total_score_one_rounds_right_column_mode_move(
    round_input: str,
    opponent_move: process.RPSShapes,
    your_move: process.RPSShapes,
    total_score: int,
) -> None:
...
"""
    rps = process.RockPaperScissors(encrypted_strategy_guide, right_column_mode="move")
    assert [
        (round.your_move.value + round.outcome.value) for round in rps.strategy_guide
    ] == [8, 1, 6]
...
def test_total_score_one_rounds_right_column_mode_outcome(
    round_input: str,
    opponent_move: process.RPSShapes,
    outcome: process.RPSScores,
    total_score: int,
) -> None:
    encrypted_strategy_guide = f"""\
{round_input}
"""
    rps = process.RockPaperScissors(
        encrypted_strategy_guide, right_column_mode="outcome"
    )
    assert len(rps.strategy_guide) == 1
    round = rps.strategy_guide[0]
